...
def edit_resena(resena_id):
    ...
    if id_usuario_calificador != session['user_id']:
        ...
        cursor.close()
        return redirect(url_for('deptos.view_property', property_id=id_departamento))
...
def delete_resena(resena_id):
    ...
    try:
        ...
        cursor.close()
        return jsonify({'success': True, 'message': 'Reseña eliminada', 'redirect_url': url_for('deptos.view_property', property_id=id_departamento)})
    except Exception as e:
        mysql.connection.rollback()
...
def mark_all_notifications_read():
    if 'user_id' not in session:
        ...
        return redirect(url_for('auth.login')) # O devolver JSON si es para AJAX
